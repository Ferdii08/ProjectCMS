...
@section('content')
    <h2 style="margin-bottom: 16px;">Tambah Staff Baru</h2>

    <form method="POST" action="{{ route('staff.store') }}" style="line-height: 2;">
        @csrf
        ...
        <label>Nomor Telepon: <input type="text" name="no_telepon" value="{{ old('no_telepon') }}" required></label><br><br>
        @error('no_telepon')
            <span style="color: red;">{{ $message }}</span><br><br>
        @enderror
